...
$datenow = date("d-F-Y");
	...
	$gclass = "
				SELECT concat(b.firstname,' ',b.lastname) name , a.student_id 
												FROM `discipline_bdg_pri_yellowform` a
...
echo"<page backtop='29mm' backbottom='-1mm' backleft='-1mm' backright='-1mm'>
			...
			<td>&nbsp;</td><td>&nbsp;</td>
			<td  ><span   style='font-size:16px; ' ><b>Bina Bangsa School - Bandung Primary </b><br> Discipline Report Term ".$_POST['term']." ( 20".$ayx."/20".$ay." )</span></td>
			 ";
			...
			be an Excellent way to communicate and convey information to the parents using the yellow discipline notice. We appreciate your support and if you have questions please call the school at (021) 588 0045. 
			</p>
			<i>
...
left join (select b.*,a.code from discipline_bdg_pri_subcat  a left join discipline_bdg_pri_cat b on a.cat_id=b.id ) h on a.code=h.code
...
order by id desc
				...
				echo"<td style='width:220px;'>".$rdate2['offence_details']." </td>";
...
SELECT  concat(g.firstname) teacher,concat(f.firstname,' ',f.lastname) student,e.initials room,d.initials,a.*
  ...
  FROM `discipline_bdg_pri_deduction` a 
left join classstudents b on a.student_id=b.student_id 
left join classrooms c on c.id=b.classroom_id
left join levels d on d.id=c.level_id
left join rooms e on e.id=c.room_id
...
order by id desc
				...
				echo"<td style='width:220px;'>".$rdate21['offence_details']." </td>";
			...
			echo"<tr >";
			echo"<td style='text-align:center;'>Parent</td>";
			echo"<td >&nbsp;</td>";
