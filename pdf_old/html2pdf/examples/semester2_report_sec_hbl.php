	...
	while($rp = $sp->fetch_assoc())
	{
		...
		$qall = "SELECT C.firstname as fname, B.code as Student_Code, C.id as Student_ID, D.id as Subject_ID, D.name AS Subject, G.name as Class, H.name as Cohort, F.id as campusid, F.name AS campus, A.ct1 as BMT1, A.cat1 as CAT1, A.mye AS MYE, A.ca1 as CA1, round(A.sa1,2) AS SA1,
				...
				while($rall = $sall->fetch_assoc())
				{
					...
					JOIN profiles P ON(P.user_id = E.staff_id)
					JOIN users Z on (Z.id = B.user_id)
					WHERE E.academicyear_id ='".$ay_id."' AND D.category=1 AND C.id = '".$studentid."' AND A.fya > 0 and E.classroom_id='".$classroomid."' && Z.status=1 && A.subject_id != '588'
					ORDER BY D.priority";
					}
					$sum = 0;
					...
					while($rsall = $ssall->fetch_assoc())
					{
						...
						if((substr($subject,0,8) == 'Mandarin') || (substr($subject,0,7) == 'Chinese'))
						{
							...
							if($ay_id < 20){
								...
								$qchis = "SELECT * FROM chinese_discount where id = '".$subjectid."'";
								$schis = $db->query($qchis);
								$rchis = $schis->fetch_assoc();
								if(count($rchis)==0){
									$chinesedrop = 100;
								}
								else{
									$chinesedrop = $rchis['discount'];
								}
								$sum += ($chinesedrop /100 )* $FA;
							}
...
JOIN profiles P ON(P.user_id = E.staff_id)
JOIN users Z on (Z.id = B.user_id)
WHERE J.academicyear_id ='".$ay_id."' AND D.category=1 AND H.id = '".$level_id."' AND F.id = '".$camp_id."' && Z.status=1
GROUP BY C.id
ORDER BY C.firstname";
	...
	while($r = $s->fetch_assoc())
	{
		...
		if($s25->num_rows>0)
		{
			...
			while($r25 = $s25->fetch_assoc())
			{
				...
				{
					$rank[$r['Student_ID']]=$fname.' is 1<sup>st</sup> in '.$cohort.' Cohort.';
				}
				else if(($rankq[$r['Student_ID']] == 2)&& ($savg4[$r['Student_ID']]>0))
				{
					$rank[$r['Student_ID']]=$fname.' is 2<sup>nd</sup> in '.$cohort.' Cohort.';
				}
				else if(($rankq[$r['Student_ID']] == 3)&& ($savg4[$r['Student_ID']]>0))
				{
					$rank[$r['Student_ID']]=$fname.' is 3<sup>rd</sup> in '.$cohort.' Cohort.';
				}
				else
				{
					$rank[$r['Student_ID']]='';
...
JOIN profiles P ON(P.user_id = E.staff_id)
JOIN users Z on (Z.id = B.user_id)
WHERE J.academicyear_id ='".$ay_id."' AND D.category=1 AND J.id = '".$classid."' AND F.id = '".$camp_id."' && Z.status=1
GROUP BY C.id
ORDER BY C.firstname";
 ...
 <?php
   ...
   <div id="header">
   <?php if($ay_id == '19') { ?>
		<?php echo($campus."<br>".$LsStudent['Cohort']."<br>Academic Transcript"."<br>"."Semester 2"."<br>"."May 20".$ay_id."<br>"); ?> 
   <?php } else { ?>
		<?php echo($campus."<br>".$LsStudent['Cohort']."<br>Academic Transcript"."<br>"."Semester 2"."<br>"."May 20".$ay_id."<br>"); ?> 
   <?php } ?>
	</div>
	...
	<table class="tablegrade">
		<tr>
			...
			<th style='width: 70px;'>Final Exam(15%)</th>
			<th style='width: 70px;'>Sem 2 Mark(35%)</th>
       ...
       <?php 
			...
			P.firstname as Teacher, P.user_id as Teacher_ID,E.lock1 as lock1,E.lock2 as lock2,E.lock3 as lock3,E.lock4 as lock4,A.id as recid,A.ct2 as BMT2, A.cat3 as CAT3,
			A.fye as FYE,case when A.ca2 < '40' then '40' else A.ca2 end  as CA2, A.sa2 as SA2, A.fya as FA,H.id as levelid,H.chineselevel_id as chineselevelid,A.award as Medal,F.id as campusid
			, A.cat4 as CAT4
			FROM enrolls A
			JOIN students B ON ( B.id = A.student_id ) 
			JOIN profiles C ON ( C.id = B.profile_id ) 
			...
			JOIN profiles P ON(P.user_id = E.staff_id)
			JOIN users Z on (Z.id = B.user_id)
			WHERE E.academicyear_id ='".$ay_id."' AND D.category=1 AND C.id = '".$LsStudent['Student_ID']."' AND A.fya > 0 and E.classroom_id='".$classid."' && Z.status=1
			ORDER BY D.priority";
			}
			else {
            //Mark Academic
			...
			JOIN profiles P ON(P.user_id = E.staff_id)
			JOIN users Z on (Z.id = B.user_id)
			WHERE E.academicyear_id ='".$ay_id."' AND D.category=1 AND C.id = '".$LsStudent['Student_ID']."' AND A.fya > 0 and E.classroom_id='".$classid."' && Z.status=1 && A.subject_id != '588'
			ORDER BY D.priority";
		//	die($qs);
            }
             ...
             while($rs = $ss->fetch_assoc())
				{
					...
					$FA = ((($CA1 * 0.2) + ($MYE * 0.25) + ($CA2 * 0.2) + ($FYE * 0.15) ) /80) * 100;
				...
				/*	if($camp_id=10)
					...
					if($subject=="Chinese Level JC1 AS (L JC1-AS)"){
						
						$subject="Chinese Level JC1 AS <br> (L JC1-AS)";
					}
					echo("<tr>");
...
A.fye as FYE,case when A.ca2 < '40' then '40' else A.ca2 end  as CA2, A.sa2 as SA2, A.fya as FA,A.gradet3 as t3non,A.gradet1 as t1non,A.grade2 as Grade2
FROM enrolls A
JOIN students B ON ( B.id = A.student_id ) 
JOIN profiles C ON ( C.id = B.profile_id ) 
JOIN subjects D ON ( D.id = A.subject_id ) 
...
JOIN profiles P ON(P.user_id = E.staff_id)
JOIN users Z on (Z.id = B.user_id)
WHERE E.academicyear_id ='".$ay_id."' AND D.category=2 AND C.id = '".$LsStudent['Student_ID']."' and A.grade2 != '' and E.classroom_id='".$classid."' && Z.status=1
ORDER BY D.priority";
		...
		//GET CCA
			$qcca = "SELECT C.name as cca, A.grade2 as ccagrade,  AVG(A.score_final) as AVG2
					...
					WHERE B.academicyear_id = '".$ay_id."' and B.camp_id = '".$LsStudent['campusid']."' and A.student_id = '".$LsStudent['Student_ID']."' && A.score_final > 0";
					//die($qcca);
