		...
		if(count($check_token)==1){
			...
			if(count($checkUsers)==0){
				...
				if($this->input->get('device_id')){
					$param=array();
					$param['device_id']=$this->input->get('device_id');
					$param['keluarga_jemaat_id']=$keluarga_jemaat_id;
					$param['created_at']=date('Y-m-d H:i:s');

					$this->m_model->insertgetid($param,'device_users');
				}
				...
				//echo $this->input->get('device_id'); die($this->input->get('device_id'));
				if($this->input->get('device_id')){
					$keluarga_jemaat_id=$checkUsers[0]->keluarga_jemaat_id;
					//check dulu device id untuk keluarga tersebut dha pernah terdaftar apa belum
					$check=$this->m_model->selectas2('keluarga_jemaat_id', $keluarga_jemaat_id, 'device_id', $this->input->get('device_id'), 'device_users');
					
					if(count($check)==0){
						//insert data device id karena belum pernah terdaftar
						$param=array();
						$param['device_id']=$this->input->get('device_id');
						$param['keluarga_jemaat_id']=$keluarga_jemaat_id;
						$param['created_at']=date('Y-m-d H:i:s');
