	...
	<tbody>
		...
		foreach ($angjem as $key => $value) {
			...
			if(isset($sudah_vote[$value->id])){
				...
				if($sudah_vote[$value->id]->num_dipilih < 8){
					$sisa_hak_suara=8-$sudah_vote[$value->id]->num_dipilih;
					$status_numdipilih='<span style="background:orange; color:#fff;">Sisa Hak Suara</span>'.$sisa_hak_suara;
					if($sudah_vote[$value->id]->locked==0){
						$status_vote='<span style="background:gray; color:#fff;">Belum dilock</span>';
						$tidaksah=$tidaksah+1;
					}
					else if($sudah_vote[$value->id]->locked==1){
						$status_vote='<span style="background:green; color:#fff;">Sudah dilock</span>';
					}
				}
				if($sudah_vote[$value->id]->num_dipilih == 8){
					$status_numdipilih='-';
